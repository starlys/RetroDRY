...
namespace RetroDRY
{
    ...
    {
        //these lists are populated during AddDatonUsingClassAnnotation (they can't be filled any other way) and then nulled in Finalize
        ...
        {
            if (name == null) throw new Exception($"Missing name in FindDef.");
        ...
        public DatonDef? AddDatonUsingClassAnnotation(Type datonType)
        {
            ...
            {
                datondef.MainTableDef = new TableDef(datonType.Name, datonType);
        ...
        {
            if (TableInheritances == null || IncompleteSelectBehaviors == null || ColumnInheritances == null) 
                throw new Exception("Cannot call FinalizeInhertiance twice");
        ...
        private static void CopyColumnInheritance(ColDef sourceColdef, ColDef targetColdef)
        {
            ...
            targetColdef.Image ??= sourceColdef.Image; //don't overwrite
            ...
            targetColdef.Prompt ??= sourceColdef.Prompt; //don't overwrite
        ...
        {
            if (TableInheritances == null)
                throw new Exception("Cannot call PopulateTableAnnotations after finalized");
        ...
        {
            if (IncompleteSelectBehaviors == null || ColumnInheritances == null) 
                throw new Exception("Cannot call PopulateFieldAnnotations after finalized");
            ...
            foreach (var field in type.GetFields())
            {
                ...
                if (typeof(IList).IsAssignableFrom(field.FieldType))
                {
                    ...
                    var childTabledef = new TableDef(field.Name, field.FieldType.GenericTypeArguments[0]);
                    ...
                    if (isTopLevel)
                    {
                        if (datondef == null) throw new Exception("Expected datondef in PopulateFieldAnnotations");
        ...
        {
            if (TableInheritances == null)
                throw new Exception("Cannot call PopulateCriteria after finalized");
            ...
            datondef.CriteriaDef = new TableDef(ctype.Name, ctype);
        ...
        {
            dict ??= new SortedList<string, string>(1);
            dict[""] = prompt;
            return dict;
        }
